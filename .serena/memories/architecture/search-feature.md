...
## Frontend
- `store.ts` — `SearchType` union (`'tracks' | 'artists' | 'playlists' | 'albums'`), `inputValue`, `searchType`. Tiny Zustand store.
...
- `components/SearchTab.tsx` — SearchBar + Tabs (Tracks/Artists/Playlists/Albums). Hosts all four result lists. Folder picker shown only on tracks tab. Albums tab reuses `PlaylistSearchResultList` component (same `ArtistPlaylist` type).
...
- `hooks/useSearchQuery.ts` (tracks), `hooks/useArtistSearchQuery.ts` (users), `hooks/usePlaylistSearchQuery.ts` (playlists), `hooks/useAlbumSearchQuery.ts` (albums) — thin wrappers over the generic hook.
...
## Backend
- `services/search.rs` — generic `search_api::<Raw, Out>(client_id, query, limit, offset, endpoint)` hits `{API_V2_BASE}/search/{endpoint}`. `search_tracks` (endpoint "tracks") + `search_users` (endpoint "users") + `search_playlists` (endpoint "playlists_without_albums") + `search_albums` (endpoint "albums"). Public response types `SearchResponse`, `UserSearchResponse`, `PlaylistSearchResponse` with `collection` + `total_results`. `RawPlaylistSearchItem` uses `#[serde(flatten)]` on `ArtistPlaylist` base with artwork fallback to first track. `search_albums` reuses `PlaylistSearchResponse` and `ArtistPlaylist` — no new model needed.
- `commands/search.rs` — `search_tracks`, `search_users`, `search_playlists`, `search_albums` tauri commands; fetch client_id then call service.
...
## i18n
- `search.*` keys in en.json/fr.json: tabTracks, tabArtists, tabPlaylists, tabAlbums, placeholder(Artists/Playlists/Albums), emptyState(Artists/Playlists/Albums), noArtistResults, noPlaylistResults, noAlbumResults, noResults, errorSearch, followers, tracks, playlistTrackCount, loadingMore.
