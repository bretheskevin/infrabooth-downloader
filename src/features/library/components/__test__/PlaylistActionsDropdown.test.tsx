...
vi.mock('@/hooks/useDeletePlaylist', () => ({
...
describe('PlaylistActionsDropdown', () => {
  ...
  it('disables button when disabled prop is true', () => {
    render(<PlaylistActionsDropdown tracks={[mockTrack]} playlistName="My Playlist" disabled />);
    expect(screen.getByRole('button')).toBeDisabled();
  });

  it('renders nothing when rekordbox is not found', () => {
    mockDetectionData = { found: false, version: null, dbPath: null, isRunning: false };
    const { container } = render(<PlaylistActionsDropdown tracks={[mockTrack]} playlistName="My Playlist" />);
  ...
  describe('defaultFolderId priority chain', () => {
    ...
    it('prefers playlist parent over stored default', async () => {
      ...
      render(
        <TooltipProvider>
          <PlaylistActionsDropdown tracks={[mockTrack]} playlistName="My Playlist" />
        </TooltipProvider>,
      );
      await user.click(screen.getByText('start'));
  ...
  describe('delete playlist action', () => {
    const deleteAction = { playlistId: 123 };

    it('shows delete menu item when deleteAction is provided', async () => {
      const user = userEvent.setup();
    ...
    it('does not show delete menu item without deleteAction', async () => {
      const user = userEvent.setup();
      render(<PlaylistActionsDropdown tracks={[mockTrack]} playlistName="My Playlist" />);
      await user.click(screen.getByRole('button'));
    ...
    it('calls deletePlaylist with playlistId when confirm is clicked', async () => {
      const user = userEvent.setup();
      render(<PlaylistActionsDropdown tracks={[mockTrack]} playlistName="My Playlist" deleteAction={deleteAction} />);
      await user.click(screen.getByRole('button'));
      await user.click(screen.getByText('delete'));
      await user.click(screen.getByText('deleteConfirm'));
