...
export const TrackCard = memo(function TrackCard({ track, isCurrentTrack, isInitializing = false }: TrackCardProps) {
  ...
  const statusBadge = <TrackStatusBadge status={track.status} error={track.error} className="flex-shrink-0" />;

  const initializingBadge = (
    <div role="status" aria-live="polite" className="flex items-center gap-1.5 flex-shrink-0">
      <Loader2 role="img" aria-label={t('download.startingDownload')} className="h-4 w-4 flex-shrink-0 text-primary animate-spin" />
      <span className="text-xs font-medium text-primary">{t('download.startingDownload')}</span>
    </div>
  );
  ...
  return (
    ...
    >
      <Avatar className="h-12 w-12 rounded-lg flex-shrink-0 shadow-sm">
        {track.artworkUrl ? (
          <AvatarImage src={track.artworkUrl} alt={track.title} className="rounded-lg object-cover" loading="lazy" />
        ) : null}
        <AvatarFallback className="rounded-lg bg-secondary" data-testid="artwork-fallback">
          <Music className="h-5 w-5 text-muted-foreground" aria-hidden="true" />
        </AvatarFallback>
      </Avatar>

      <div className="flex-1 min-w-0">
        <p className={cn('text-sm font-medium truncate', (isActive || showInitializing) && 'text-primary')}>{track.title}</p>
