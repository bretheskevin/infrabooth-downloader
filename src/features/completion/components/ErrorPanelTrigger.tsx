...
export function ErrorPanelTrigger({ failedCount, onClick, isExpanded = false }: ErrorPanelTriggerProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        <span className="text-sm font-medium text-foreground">{t('completion.tracksNeedAttention', { count: failedCount })}</span>
      </div>

      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
        <span>{isExpanded ? t('errors.closePanel') : t('completion.viewFailed')}</span>
        <ChevronDown className={cn('h-3.5 w-3.5 transition-transform duration-200', isExpanded && 'rotate-180')} aria-hidden="true" />
      </div>
    </button>
  );
}
