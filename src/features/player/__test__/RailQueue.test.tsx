...
vi.mock('react-i18next', () => ({
  useTranslation: () => ({
    t: (key: string, opts?: { count?: number }) => {
      ...
      if (key === 'player.queueCount' || key === 'player.queueCount_one' || key === 'player.queueCount_other')
...
vi.mock('@/components/ui/virtual-list', () => ({
  ...
  VirtualRow: ({ children }: { children: React.ReactNode }) => <div>{children}</div>,
}));

vi.mock('../components/QueuePanelItem', () => ({
  QueuePanelItem: ({ item, isCurrent }: { item: { title: string }; isCurrent: boolean }) => (
...
vi.mock('../store', () => ({
  usePlayerStore: (selector: (s: typeof mockState) => unknown) => selector(mockState),
}));
...
describe('RailQueue', () => {
  ...
  it('renders DndContext when queue has items', () => {
    ...
    vi.mocked(useVirtualizedList).mockReturnValue({
      parentRef: { current: null },
      virtualItems: [{ index: 0, size: 44, start: 0, key: 0, end: 44, lane: 0 }],
      totalSize: 44,
      getScrollOffset: () => 0,
    });
