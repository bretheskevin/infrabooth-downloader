...
export function AffiliationRow({ item, onClose }: AffiliationRowProps) {
  ...
  return (
    <button type="button" onClick={() => openActorProfile(item.actor, onClose)} className={NOTIFICATION_ROW_CLASS}>
      <img src={item.actor.avatar_url ?? undefined} alt="" className="h-8 w-8 rounded-full bg-muted shrink-0 object-cover" />
