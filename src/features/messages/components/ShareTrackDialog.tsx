...
export function ShareTrackDialog() {
  ...
  useEffect(() => {
    if (shareTrack && hasNextPage && !isFetchingNextPage) {
      void fetchNextPage();
    }
  }, [shareTrack, hasNextPage, isFetchingNextPage, fetchNextPage]);
  ...
  const handleSend = () => {
    if (!selectedUserId || !shareTrack) return;
    const trimmed = messageText.trim();
    const content = trimmed
      ? `${trimmed}\n${shareTrack.permalinkUrl}`
      : shareTrack.permalinkUrl;
  ...
  return (
    <Dialog open={shareTrack !== null} onOpenChange={(open) => { if (!open) handleClose(); }}>
      <DialogContent className="sm:max-w-[400px] p-0 gap-0">
        ...
        <div ref={parentRef} className="h-[240px] overflow-y-auto px-2">
          {isLoading && (
