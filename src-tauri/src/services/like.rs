...
use crate::services::http::{check_api_success, RequestBuilderExt, API_V2_BASE, HTTP_CLIENT, SC_APP_VERSION};
...
) -> Result<(), LikeTrackError> {
    log::info!("[like] Liking track {}", track_id);

    let response = HTTP_CLIENT
        .put(track_like_url(current_user_id, track_id, client_id)?)
...
pub async fn unlike_track(
    ...
    let response = HTTP_CLIENT
        .delete(track_like_url(current_user_id, track_id, client_id)?)
        .with_oauth(Some(oauth_token))
        .with_datadome(datadome)
        .send()
        .await?;
