...
pub fn create_backup(db_dir: &Path, app_data_dir: &Path) -> Result<PathBuf, RekordboxError> {
    ...
    if db_file.exists() {
        fs::copy(&db_file, backup_path.join("master.db"))
    ...
    if xml_file.exists() {
        fs::copy(&xml_file, backup_path.join("masterPlaylists6.xml"))
