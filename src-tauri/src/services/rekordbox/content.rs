...
use super::models::{FILE_TYPE_MP3, TrackMetadata};
...
pub fn add_content(
    ...
    let album_id = metadata
        .album
        .as_ref()
        .map(|name| resolve_or_create_album(db, name))
        .transpose()?;
    ...
    let search_str = build_search_str(
        &metadata.title,
        &metadata.artist,
...
fn resolve_or_create_named_entity(
    db: &mut RekordboxDatabase,
    table: &str,
    name: &str,
) -> Result<String, RekordboxError> {
...
pub fn resolve_or_create_artist(
    db: &mut RekordboxDatabase,
    name: &str,
) -> Result<String, RekordboxError> {
...
fn find_content_id_by_path(
    db: &RekordboxDatabase,
    path: &str,
) -> Result<Option<String>, RekordboxError> {
    db.conn()
        .query_row(
            "SELECT ID FROM djmdContent WHERE FolderPath = ?1",
            params![path],
            |row| row.get(0),
        )
        .optional()
...
fn get_device_info(db: &RekordboxDatabase) -> Result<(String, String), RekordboxError> {
    db.conn()
        .query_row(
            "SELECT ID, MasterDBID FROM djmdDevice LIMIT 1",
            [],
...
fn get_content_link(db: &RekordboxDatabase) -> Result<i64, RekordboxError> {
    db.conn()
        .query_row(
            "SELECT rb_local_usn FROM djmdMenuItems WHERE Name = 'TRACK'",
            [],
            |row| row.get(0),
        )
