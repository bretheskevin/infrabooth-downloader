...
async fn resolve_track_sources(
    app: &tauri::AppHandle, tracks: Vec<TrackCore>, output_dir: &Path, total: u32,
    ...
    let mut pairs: Vec<(TrackCore, PathBuf)> = Vec::with_capacity(tracks.len());
    ...
    for (idx, track) in tracks.into_iter().enumerate() {
        ...
        emit_progress(
            ...
            RekordboxExportProgressEvent {
                ...
                status: RekordboxExportStatus::Downloading,
        ...
        let source_path = if let Some(path) = existing.get(&track.track_id) {
            path.clone()
        } else {
            ...
            match download_and_convert(app, config, None).await {
                Ok(path) => path,
                Err(e) => {
                    let msg = format!("{}: download failed — {}", track.title, e);
                    emit_progress(
                        app,
                        RekordboxExportProgressEvent {
                            track_id: track.track_id.clone(),
                            track_title: track.title.clone(),
                            status: RekordboxExportStatus::Error,
                            current_track: current,
                            total_tracks: total,
...
pub async fn export_playlist_to_rekordbox(
    tracks: Vec<TrackCore>, playlist_name: String, manual_db_path: Option<String>, app: tauri::AppHandle,
    ...
    let (resolved, download_errors) = resolve_track_sources(&app, tracks, &export_dl_dir, total).await?;
            ...
            for (idx, (track, source_path)) in resolved.iter().enumerate() {
                emit_progress(
                    ...
                    RekordboxExportProgressEvent {
                        track_id: track.track_id.clone(),
                        track_title: track.title.clone(),
                        status: RekordboxExportStatus::Exporting,
                        current_track: idx as u32 + 1,
                        total_tracks: total,
                        error: None,
                    },
                );
