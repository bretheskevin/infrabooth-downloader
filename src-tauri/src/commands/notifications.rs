...
use crate::services::notifications::{self, LastSeenActivityState, NotificationsCache, NotificationsPage, UnreadCountResult};
...
pub async fn get_unread_count(app: tauri::AppHandle) -> Result<UnreadCountResult, String> {
    ...
    let latest_ts = match cache.get_unread_probe() {
        Some(ts) => ts,
        None => {
...
pub async fn get_notifications_page(app: tauri::AppHandle, cursor: Option<String>) -> Result<NotificationsPage, String> {
    ...
    if cursor.is_none() {
        if let Some(cached) = cache.get_first_page() {
            return Ok(cached);
        }
    }
