...
Rules:
- **ALWAYS use graphify first when searching for code.** Whenever I ask you to find, locate, or understand related code files for something, run `graphify query "<question>"` BEFORE reaching for grep/glob/Read or any subagent search. Only fall back to raw search tools if graphify does not surface enough context.
